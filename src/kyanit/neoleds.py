...
class NeoLeds:
    ...
    def display(self, colors, anim=None, anim_speed=10, time=None, brightness=1):
        ...
        self._colors[0 if time is None else 1] = (
            colors
            if brightness == 1
            else [[int(comp * brightness) for comp in color] for color in colors]
        )
        self._anim[0 if time is None else 1] = anim
        self._temp_until = (
            None if time is None else utime.ticks_add(utime.ticks_ms(), time * 1000)
        )
    ...
    async def refresh_leds(self):
        ...
        while True:
            ...
            if self._colors[disp] is not None:
                self._write_colors(
                    self._colors[disp]
                    if self._anim[disp] is None
                    else
                    # call anim with the phase and colors
                    self._anim[disp](self._anim_phase, self._colors[disp])
                )
...
class Animations:
    ...
    def breathe(phase, colors):
        ...
        return [
            [
                int(abs(component / (2 + math.sin(math.pi * 2 * phase / 500))))
    ...
    def attention(phase, colors):
        ...
        return [
            [int(abs(component / anim(phase))) for component in color]
            for color in colors
        ]
