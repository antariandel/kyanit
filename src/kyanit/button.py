...
This module publishes the `Button` class, which can be used to monitor a button and
...
class Button:
    """
    Instantiate this class with a pin number to catch button-press events on the pin.
    The pin number is the GPIO number on the controller.

    If the button is active-high (ie. it's logical high on press, and logical low by
    default), set `invert` to `False`.

    Once instantiated, the `monitor` task must be started for button monitorization.
    After that, check the last button press event with the `check` function.

    NOTE: You should check button events frequently, because if more than 1 event occurs
    between checks, only the last event will be returned. Frequent checks will also make
    the button more responsive. A 100ms wait between checks should be a good choice.
    """

    def __init__(self, pin_number, invert=True):
        self._button_signal = machine.Signal(
            machine.Pin(pin_number, machine.Pin.IN), invert=invert
        )
    ...
    async def monitor(self):
        """
        This task continuously monitors the button. It must be started for `check` to
        work.
        """
