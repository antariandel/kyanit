...
Kyanit Core is a user code supervisor built for MicroPython. Development is currently focused on the
...
Being a supervisor, Kyanit Core is responsible for starting and stopping user code, as well as
handling any uncaught exceptions in the user code.
...
The Kyanit Core package is intended to be compiled into the MicroPython firmware, and flashed into
...
is required, for changing the pin numbers, see `kyanit.controls`.
...
To flash a released version into an ESP8266, see the
[*Flashing a Released version of Kyanit Core*](#flashing-a-released-version-of-kyanit-core) section
...
The first time you power up the board, it will set up an AP with an SSID starting with `Kyanit` and
ending with a unique hexadecimal number. When the AP is active, the LEDs will flash blue.
...
Follow the instructions and enter your home network SSID and password. You can review your settings
before it gets uploaded to the board.
...
With a fresh install, you should see `wlan.json`, which includes your wireless credentials and IP
settings, and `code.py`, which is the user code that was uploaded as part of the initial setup.
...
recurse to further directories inside).
...
Kyanit Core is built around MicroPython's uasyncio. (CPython's asyncio implementation for
MicroPython.) This means Kyanit is all about coroutines. Albeit it's advisable to learn about
coroutines before coding on Kyanit, it's not entirely required, if you abide by some basic rules.
...
round-robin fashion, and will return to the function as soon as possible.
...
All of these modules are available for the user code. Check out their documentation page to see what
they can do.

## Kyanit Netvar

Kyanit Core has a notion of a "network variable"which is available to read and write through the
network connection by Kyanit CTL or Kyanit API.

The Netvar can be used to issue custom commands to your board, such as switching or reading some
data from your board (like sensor data).

Check out the `kyanit.Netvar` class for details.

# Kyanit API

So far it was demonstrated how Kyanit CTL can be used to control a Kyanit board. If you want to
control your board from Python code, check out Kyanit API at https://kyanit.eu/docs/kyanit-api.
...
class RebootError(Exception):
    """
    This exception is passed to `code.cleanup` on a `reboot` request through Kyanit API or Kyanit
    CTL.
    """
    pass
...
#     """
#     This exception is passed to `code.cleanup` on a `reset` request through Kyanit API or Kyanit
#     CTL.
#     """
#     pass
...
class Netvar:
    """
    The Netvar is a notion of a "network variable", which is available to read and write through
    ...
    `Netvar.outbound()` on the other hand is what Kyanit publishes, and is available for reading by
    Kyanit CTL or Kyanit API.

    Both methods have the same usage. Having no parameters passed gets the actual value of the
    variables:
    ...
    As suggested by the example, the inbound variable might be overwritten when the data (or part of
    ...
    Setting and getting the netvar is realized through HTTP, so keep in mind that there's an order
...
def get_color_id():
    ...
    Returned value will be 'BBB' (address of 0) if Kyanit can not connect to the wireless network,
    or if it loses connection.
    """
...
def controls(kyanit_leds=None,
    ...
    Having this decorator on `code.main` and `code.cleanup` gives visual feedback on run state, as
    well as showing the Color ID on the LEDs, when the button is pressed.

    When user code is running, the LEDs will continuously show the `active_colors` with a
    "breathing" animation. When code is stopped, the `idle_colors` will be shown with a slower
    "breathing" animation.
